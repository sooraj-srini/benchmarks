...
let write_heavy_workload num_elems num_threads = 
  let sl = LockedSkiplist.create () in
  ...
  let push = (fun () -> Domain.spawn ( fun () ->
    ...
    for i = 0 to (num_elems - 1) do ( 
      if (i/2) < num_elems/2 then 
        LockedSkiplist.add sl (i) |> ignore
    else
        LockedSkiplist.remove sl (i - (num_elems/2)) |> ignore)
    done;
    start_time
  )) in 
  ...
  let push = (fun () -> Domain.spawn ( fun () ->
    ...
    for i = 0 to (num_elems - 1) do ( 
      ...
      if i mod 1000 < 90 then 
        LockedSkiplist.add sl elems.(i) |> ignore
      else if i mod 1000 >= 90 && i mod 1000 < 100 then 
        LockedSkiplist.remove sl elems.(i) |> ignore
      else 
        LockedSkiplist.find sl elems.(i) |> ignore
    )
    done;
    start_time
  )) in 
  ...
  let moderate_heavy_workload num_elems num_threads = 
    ...
    let push = (fun () -> Domain.spawn ( fun () ->
      ...
      for i = 0 to (num_elems - 1) do ( 
        ...
        if prob < 0.2 then 
          LockedSkiplist.add sl (Random.int 10000) |> ignore
        else if prob >= 0.2 && prob < 0.3 then 
          LockedSkiplist.remove sl (Random.int 10000) |> ignore
        else 
          LockedSkiplist.find sl elems.(i) |> ignore
      )
      done;
...
let bench ~workload_type ~num_elems ~num_threads () =
  ...
  Benchmark_result.create_generic ~median_time ~median_throughput ("locked_skiplist_"^workload_type)