...
let add slist key =
  ...
  let rec assign index =
    ...
    if index == 0 then true else assign (index - 1)
  in
  let return_val = if not pres then assign (height - 1) else false in 
  Mutex.unlock slist.lock;
  return_val
...
let remove slist key =
  ...
  let rec reassign key index =
    ...
    if index == 0 then true else reassign key (index - 1)
  in
  let return_val = if pres then reassign key (max_height - 1) else false in
  Mutex.unlock slist.lock;
  return_val
