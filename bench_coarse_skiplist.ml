let workload num_elems num_threads add remove = 
  let sl = Coarseskiplist.create () in
  ...
  let push = (fun () -> Domain.spawn ( fun () ->
    ...
    for i = 0 to (num_elems - 1)/num_threads do ( 
      ...
      if prob < add then 
        Coarseskiplist.add sl (Random.int 10000) |> ignore
      else if prob >= add && prob < add +. remove then 
        Coarseskiplist.remove sl (Random.int 10000) |> ignore
      else 
        Coarseskiplist.find sl elems.(i) |> ignore
    )
    done;
    start_time
  )) in 
...
let bench ~workload_type ~num_elems ~num_threads () =
  let workload =
    ...
    else if workload_type = "balanced_heavy" then
      balanced_heavy_workload
    else 
  ...
  Benchmark_result.create_generic ~median_time ~median_throughput ("coarse_locked_skiplist_"^workload_type)